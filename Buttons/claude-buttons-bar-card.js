...
(function () {
  ...
  class ClaudeButtonsBarCard extends HTMLElement {
    connectedCallback() {
      if (!this.shadowRoot) this.attachShadow({ mode: 'open' });
    }

    setConfig(config) {
      if (!config.buttons?.length) throw new Error('claude-buttons-bar-card: "buttons" array is required');
  ...
  customElements.define('claude-buttons-bar-card', ClaudeButtonsBarCard);

  window.customCards = window.customCards || [];
  window.customCards.push({
    type:        'claude-buttons-bar-card',
