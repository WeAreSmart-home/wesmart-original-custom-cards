// wesmart-infinite-color-card.js v1.0.0
// WeSmart — Infinite Color History Card for Home Assistant
// Generates a full color palette from a single base color using HSL algorithm
...
(() => {
  ...
  const CARD_TAG     = 'wesmart-infinite-color-card';
  const EDITOR_TAG   = 'wesmart-infinite-color-card-editor';
  ...
  // No hardcoded palette here — everything is driven by the generated palette.
  ...
  class WeSmartInfiniteColorCard extends HTMLElement {
    constructor() {
      super();
    ...
    /**
     * Builds a full design token palette from the base color and theme mode.
     *
     * Dark theme: keeps hue as accent, generates very dark tinted backgrounds.
     * Light theme: slightly darkens accent for readability, generates near-white tinted backgrounds.
     */
    ...
    /**
     * Computes the palette for the current config color + theme and
     * injects all tokens as CSS custom properties on the host element.
     * Because custom properties are inherited, the shadow DOM picks them up.
     */
    _applyPalette() {
    ...
    setConfig(config) {
      if (!config.entities || !config.entities.length) {
        throw new Error('wesmart-infinite-color-card: "entities" is required');
      }
      this._config = {
      ...
      // For auto theme, re-apply palette when system preference changes
      if (this._mqHandler) {
    ...
    _updateCurrentStates() {
      ...
      this._entities.forEach((cfg, i) => {
        ...
        if (iconEl) {
          iconEl.style.color = isOn ? 'var(--accent)' : '';
    ...
    async _fetchHistory() {
      ...
      try {
        ...
        console.error('[wesmart-infinite-color-card] fetch error:', err);
    ...
    _renderLineChart(history, entityId) {
      ...
      // Unique gradient ID per entity to avoid clashes across multiple cards
      const gradId = `wic-${entityId.replace(/\W/g, '-')}`;
      const accent = this._palette.accent ?? '#D97757';
  ...
  class WeSmartInfiniteColorCardEditor extends HTMLElement {
  ...
  customElements.define(CARD_TAG, WeSmartInfiniteColorCard);
  customElements.define(EDITOR_TAG, WeSmartInfiniteColorCardEditor);

  window.customCards = window.customCards || [];
  window.customCards.push({
    type:        CARD_TAG,
    name:        'WeSmart Infinite Color Card',
    description: 'History card with dynamic palette generated from a single base color — timeline for binary sensors, line chart for numeric sensors.',
    preview:     false,
  });

  console.info(
    `%c WESMART INFINITE COLOR %c v${CARD_VERSION} `,
